<template>
    <el-scrollbar class="chapter-card">
        <div  class="word-card" :class="{ 'active': store.currentChapterWordIndex === word_index }"
            ...
            <div v-show="!(store.hideGraspWords && store.isGrasp(item))&& !(store.hideCollectWords && store.isCollect(item))" class="word-card-item">
                <div class="word-card-item-title">
                    <div class="word-card-item-title-name">{{ item.name }}</div>
                    <div class="word-card-item-title-icon">
                        <el-icon >
                            <CircleCheckFilled v-if="store.isGrasp(item)" @click.stop="handleUnGrasp(item)" />
                            <CircleCheck v-else @click.stop="handleGrasp(item)" />
                        </el-icon>
                        <el-icon>
                            <StarFilled v-if="store.isCollect(item)" @click.stop="handleUnCollect(item)" />
...
.active {
    box-shadow: 0px 0px 6px 0px var(--el-color-primary);
}
